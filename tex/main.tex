...
Using the definitions of the blocks $B$, vehicle tasks $VT$ and crew duties $CD$, we can formulate a linear program to describe our integrated problem. In this, let $x_i$ and $y_j$ be binary variables indicating the usage of $v_i \in VT$ and $c_j \in CD$ respectively. Additionally, let $k_{i,t}$ indicate that $v_i$ covers trip $t \in \mathcal{T}$, let $l_{i,b}$ indicate that $v_i$ contains block $b \in B$, and let with $m_{j,b} $ indicate that $c_j$ includes block $b \in B$. Let $\rho_{j,\text{broken}}$ and $\rho_{j,\text{between}}$ indicate whether a duty $c_j$ is of type \textit{broken} or \textit{between} respectively. In this, $k_{i,t}$, $l_{i,b}$, $m_{j,b}$ and $\rho_{j,\chi}$  are all be binary parameters. Lastly, let and let $d_j$ be a parameter indicating the duration of duty $c_j$, and let $d_{j,\textit{long}}$ be a binary parameter indicating whether $d_j > 8.5$ hours. Our cost minimization can then be formulated as follows:
\begin{align}
\min \quad
& \sum_{1 \leq i \leq |VT|} x_{i} \cdot cost(v_i) + \sum_{1 \leq j \leq |CD|} y_{j} \cdot cost(c_j)  
\end{align}
Subject to:
\begin{align}
\sum_{1 \leq i \leq |VT|} x_{i}k_{i,t} &= 1 && \forall t = 1,\:\dots,\:|T| \label{form:pd-all-trips-covered} \\
\sum_{1 \leq i \leq |VT|}x_i l_{i,b} - \sum_{1 \leq j \leq |CD|}y_j m_{j,b} &= 0 && \forall b \in B \label{form:pd-all-blocks-covered} \\
...
Subject to:
\begin{align}
\sum_{1 \leq i \leq |V|} x_{i}k_{i,t} &>= 1 && \forall t = 1,\:\dots,\:|T| \label{form:methods-integrated-all-trips-covered} \\
\sum_{1 \leq i \leq |V|}x_i l_{i,b} - \sum_{1 \leq j \leq |C|}y_j m_{j,b} &<= 0 && \forall b \in B \label{form:methods-integrated-all-blocks-covered} \\
...
x_{i} &\in \{ 0, 1 \} && \forall i = 1,\:\dots,\:|VT| \\
y_{j} &\in \mathcal{N} && \forall j = 1,\:\dots,\:|CD|
\end{align}
In this, constraint (\ref{form:methods-integrated-all-trips-covered}) ensures that all trips are covered by at least one vehicle task, while constraint (\ref{form:methods-integrated-all-blocks-covered}) ensures that all blocks are covered by exactly one vehicle task and one crew duty. Constraints (\ref{form:methods-integrated-max-avg-shift-length}-\ref{form:methods-integrated-max-between-shifts}) are identical to those previously defined in the CSP section, ensuring that the overall set of duties satisfies the labor regulations.
