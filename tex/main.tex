...
\subparagraph{E-VSP Labeling} Let us recall the reduced cost formula for a vehicle task $vt_0$ as described by Formula (\ref{form:evcsp-rc-vt}). As mentioned earlier, the difference between this and the reduced cost formula in the sequential problem is the inclusion of the dual costs of the blocks which are contained within the vehicle task. Let us visualise this within our graph $G_{vt}$ by outlining known blocks within the graph; see Figure \ref{fig:evsp-graph-blocks} for an example. In this figure, we represent the ends of trips as a nodes, and the connectings arcs incorporate both movements and (charging) idles.
...
\begin{figure}[h]
  ...
  \draw[dash pattern={on 1pt off 1pt}, thick, rounded corners=8pt]
    ($(A.west) + (0,-24mm) +(0mm,6mm)$) -- ($(B.north west)+(-3mm,3mm)$) -- ($(B.north east)+(3mm,3mm)$) -- ($(C.north west)+(-3mm,3mm)$) -- ($(C.north east)+(0mm,3mm)$) -- ($(C.south east)+(0mm,-3mm)$) -- ($(C.south west)+(-3mm,-3mm)$) -- ($(B.south east)+(3mm,-3mm)$) -- ($(B.south west)+(-3mm,-3mm)$) -- ($(A.west) + (0,-24mm) +(0mm,-6mm)$) -- cycle;
