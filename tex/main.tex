...
For the CSP, we again provided a set covering ILP with additional constraints in order to solve the problem. Two methods were now used in order to create columns: another local search to create an initial solution, and another labeling algorithm which could be used in order to solve the pricing problem. \todo{Aanpassen afhankelijk van test CSP LS} We found that the local search often performed poorly on our given instances, and therefore the labeling algorithm was primarily used in order to generate candidate crew duties. 
