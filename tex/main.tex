...
Let $T$ be a set of trips that need to be driven, let $D$ be a set of depots that can be used, and let $L$ be a set of locations. A trip is defined as a travel between two locations in $L$; let $l_s(t) \in L$ and $l_e(t) \in L$ represent the starting and ending location of trip $t$ respectively, and let $l(d) \in L$ represent the location of depot $d$. Next, let $p_s(t)$ and $p_e(t)$ refer to the planned starting and ending time of trip $t$, and let $d(t) = p_e(t) - p_s(t)$ be the duration of the trip. \\
As multiple vehicles types may be in use, let $\Phi$ represent the set of all vehicle types. For each trip $t$, a subset of these vehicle types $\phi(t) \subseteq \Phi$ may be used in order to drive the trip. Each depot $d$ also has a preallocated number of each vehicle type $\phi \in \Phi$ available; let this be represented by $g(d, \phi) \in \mathbb{N}^+$. Different vehicles might have different battery capacities; let $\sigma_{min}(\phi)$ and $\sigma_{max}(\phi)$ represent the minimal and maximal SoC of vehicle type $\phi$, both expressed in KWh. Additionally, let $\sigma_{start}(\phi)$ represent the capacity that a vehicle starts with at the beginning of a day. \\
Now, let us define given metrics when traveling between different locations. As the exact travel between locations may differ depending on the time of day (such as longer driving times when dealing with congestion), let $m$ refer to the moment of departure in the following notation. Let $d_m(l, l')$ represent the duration of the drive between $l$ and $l'$. Similarly, let $k_m(l, l')$ represent the amount of kilometers traveled, and let $c^\phi_m(l, l')$ represent the amount of battery charge used when traveling between $l$ and $l'$. Note that this last metric is dependent on the vehicle type $\phi$, as different vehicle types might have different usage patterns. For the sake of brevity, this notation will often be shortened when $l$, $l'$ and $m$ can be inferred from the context. As an example, the duration of driving from trip $t$ to trip $t'$ will be shortened to $d(t, t')$ instead of $d_{k_e(t)}(l_e(t), l_s(t'))$. \\
Lastly, let us define our recharging behavior. We will assume that it is only possible for a vehicle to charge between trips; we will therefore define a maximum amount of charge that can be gained when between trips $t$ and $t'$, while still being able to travel from $t$ to $t'$ as well. Additionally, we will assume that recharging takes place at the end of trip $t$. Charging at the beginning of $t'$ can be incorporated by adding a dummy trip with duration 0 at the location $t'$, and will not be discussed further.\todo{beter verwoorden}\\
As charging is a nonlinear process, the starting SoC $\sigma$ of a vehicle is also of importance, as well as the vehicle type $\phi$. We therefore define $u^\phi_\sigma(t, t')$ as being this maximum possible amount of charge. Note that this value can be determined beforehand when considering discrete values of $\sigma$. \\\\
...
\end{align}
Trips may only be driven sequentially if there is time to travel from one trip to the next. Note that we make the assumption that there is always time to drive to and from a depot action at the start and end of the day. 
\begin{align}
