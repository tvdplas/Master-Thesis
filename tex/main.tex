...
\begin{table}[h]
  ...
  \begin{tabular}{ll}
    ...
    $bdg(vt_i)$ & Costs due to battery degredation in $vt_i$ \\ 
...
Inspired by \citet{vanKootenNiekerk2017}, let us incorporate battery degradation by assuming that any charge action from SoC $\sigma$ to $\sigma'$ incurs some degradation related cost based on the battery in the vehicle; let this cost be decreasing in $\sigma$ and increasing in $\sigma' - \sigma$. This cost is also applicable to overnight charging.
...
\end{itemize}
Let the set of all feasible vehicle tasks be $VT$. For a vehicle task $vt$, let us define its cost as being $cost(vt) = \gamma_1 + \gamma_2 \cdot km(vt) + \gamma_3 \cdot chl(vt) + bdg(vt)$, where $km(vt)$, $chl(vt)$ and represent kilometers traveled and charge lost, $\gamma$ is a vector of cost scalars, and $bdg(vt)$ represents some additional costs based on battery degradation due to the charging schedule.
...
\node[lsnode, right=
  ...
  \end{tikzpicture}
  \caption{Three examples of edge content in $G_{vt}$. Destination of Trip 1 allows for charging, however time also allows for a detour before Trip 2. Origin of Trip 4 is a relief point, destination of Trip 3 is neither a charging location nor relief point. }
