...
\end{itemize}
Let the set of all crew duties that match these conditions be $CD$. For a crew duty $cd$ let us define its cost as being $cost(cd) = \beta_1 + \beta_2 \cdot br(c) + \beta_3 \cdot dur(cd) + typ(cd)$, where $br(cd)$ and $dur(cd)$ represent the total break time in and duration of $cd$ respectively, $typ(cd)$ represents a duty type specific cost, and $\beta$ is a vector of cost scalars. In order for a set of crew duties to be feasibly performed, additional constraints are in place:
\begin{itemize}
...
x_{i} &\in \{ 0, 1 \} && \forall vt_i \in VT
\end{align}
Here, constraint (\ref{form:all-trips-covered-seq}) once again ensures that all trips are covered by a vehicle task. Once the vehicle tasks are selected, we can use them to derive the set of blocks that are driven. Let $B_{vt} \subseteq B$ represent the set of blocks driven by a vehicle task $vt$. The overall set of blocks driven is then $B' = \bigcup_{vt_i \in VT, x_i = 1}B_{vt_i}$, and let the crew duties for which all used blocks appear in $B'$ be $CD'$. Note that in our construction of $B'$, each block is present at most once due to the fact that all trips are covered exactly once in our E-VSP formulation. \\
...
\todonocomment{loopt niet lekker} For the E-VCSP, a column generation approach in combination with a number of heuristics is once again employed in order to improve upon a solution found by using the sequential methods. As we reuse parts of the sequential approach, only the required modifications will be discussed; where applicable we will again refer back to the sequential methods for full algorithm details.
...
\begin{figure}[h]
  ...
  \node[lsnode, right=15mm of ds, yshift=10mm] (t1) {Trip 1};
  \node[lsnode, right=28mm of ds, yshift=-10mm] (t2) {Trip 2};
  \node[lsnode, right=12mm of t2, yshift=10mm] (t3) {Trip 3};
  \node[circle, draw=black, right=15mm of t3] (de) {$d_e$};
  ...
  \draw[-, dashed, rounded corners] (t1.east) -- ($(t1.east)!0.5!(t3.west) + (-1mm,-0.5mm)$) -- ($(t3.west)+(0,3mm)$);
  \draw[->, rounded corners] (t1.east) -- ($(t1.east)!0.5!(t3.west) + (0,1.5mm)$) -- ($(t3.west)+(0,3mm)$);
  \draw[->] (t2.east) -- ($(t3.west) + (0,-3mm)$);

  \draw[->, rounded corners] (t1.east) --($(t3.east) + (-2mm,7mm)$) -- (de.north west);
  \draw[->, rounded corners] (t2.east) -- ($(t3.east) + (-2mm,-7mm)$) -- (de.south west);
  \draw[->] (t3.east) -- (de.west);

  \draw[->] ($(ds) + (0,-22mm)$) -- ($(de) + (0,-22mm)$);
  \node at ($(ds) !0.5! (de) + (0,-19mm)$) {Time};
  \end{tikzpicture}
  \caption{Example of $G_{vt}$. Between Trip 1 and Trip 3, a dashed detour arc is included. Between Trip 1 and Trip 2 no time is available for a detour, so only the base arc is present.}
...
During the solving process, a set of columns $VT$ and $CD$ representing vehicle tasks and crew duties respectively are generated. Let $VT^* \subseteq VT$ and $CD^* \subseteq CD$ be the selected subsets that form the solution to the sequential problem, with $\bar{z}$ being the total cost of this solution; this provides an upper bound to the integrated problem. 

\subsubsection{Solution improvement}
We will now attempt to improve the initial solution. In order to do this, we will first discuss an ILP formulation for the E-VCSP and the problems that arise by directly using its corresponding LP relaxation. Then, we will use a Lagrangean relaxation of the ILP which we used in order to solve the E-VCSP, after which we will discuss the modifications needed to the column generations methods from Sections \ref{sec:evsp-methodology} and \ref{sec:csp-methodology} in order to use them for the integrated problem. Lastly, we will discuss additional heuristics which were applied in order to maintain computational efficiency. 

\paragraph{ILP formulation and LP relaxation} Let us once again define an ILP which can be used in order to solve the E-VCSP. We will largely continue to use the same notation as in Sections \ref{sec:problem_def}, \ref{sec:evsp-methodology} and \ref{sec:csp-methodology}. 

Let $VT$ and $CD$ be the sets of available vehicle tasks and crew duties respectively. Next, let $B_{vt}$ be the blocks covered by vehicle task $vt \in VT$, and let $B_{cd}$ be defined similarly. Let $B = \bigcup_{vt \in VT} B_{vt} \cup \bigcup_{cd \in CD} B_{cd}$ be the set of \emph{known blocks}, or put simply the set of blocks which are present in at least one generated column. In this, let us slightly relax how we define a block. From the perspective of the crew, a block is simply a working period with a start location, start time, end location and end time; the contents themselves do not matter for the feasibility of a duty, as a crew member is assumed to be working the entire time. Going forward, let us therefore consider any two blocks that start and end the same to be identical. This implies that a block may now occur more than once within a solution, however we will adjust our formulation accordingly.
...
Using these reduced cost formulations, we can now once again apply column generation to find columns with negative reduced cost which will improve the solution quality of our LP. In order to do this, we will alternate between finding vehicle and crew columns. For now, let us assume that we have column generation methods available for both vehicle tasks and crew duties; we will discuss these in more detail Section \ref{sec:evcsp-cg}. An outline of the approach is given in Algorithm \ref{alg:E-VCSP-rounds-lp}. 
\begin{algorithm}[h]
...
\end{align}
Using this, given a set of multipliers $\lambda$, we can now redefine our objective function as follows:
\begin{align}
\min \quad
...
\end{align}
Let $\lambda^*$ be the set of multipliers for which our lower bound is maximized. $\lambda^*$ can be used as a stand-in for the dual values $\pi$ of the LP relaxation discussed in the previous section, allowing us to once again use the column generation methods that we used in the sequential problem. In order to find $\lambda^*$, we use subgradient optimization. \todo{meer info over deze subgradient optimization}

Using $\lambda^*$ as a stand-in for our dual values does have a drawback: columns which are already known may be generated, as columns in the basis may get negative reduced cost. In order to minimize this effect, we will apply a modified version of a heuristic previously used in \todo{marcel, freling, caressi} which adjust the multipliers such that all selected columns have a nonnegative reduced cost, and the value of the relaxation does not decrease. We separately apply this heuristic for vehicle columns or crew columns, depending on which type of columns we will be generating in a given round.
\begin{algorithm}[h]
...
We now have a method to find a lower bound for the original E-VCSP, along with approximated dual costs for each of the constraints by letting $\pi \gets \lambda$. We will now modify Algorithm \ref{alg:E-VCSP-rounds-lp} in order to use this new approach. An outline of this process has been provided in Algorithm \ref{alg:E-VCSP-lagrange}.
...
During this process, we alternate between rounds focused on vehicle tasks and crew duties; this is done as the testing during the sequential approach showed that solution quality with a fixed set of trips / blocks significantly increased after just a few rounds, therefore multiple sequential rounds with roughly the same input may be beneficial.
...
Due to the connection with the E-VSP, we do however now have the chance to influence which blocks are chosen within vehicle tasks based on their quality in a crew solution. Given the currently described methods however, the only origin of new known blocks is the generation of vehicle tasks; the only way to influence blocks chosen within the vehicle tasks is therefore via the dual costs of the known blocks, however this does not directly encourage vehicle task generation to pick currently unknown blocks which are beneficial for the crew side. 
...
Let us consider our set of to be covered blocks $B'$ when entering a crew column generation round; each of these blocks can be traced back to its corresponding set of trips and deadheads that are driven on the vehicle side. Now consider an individual block $b \in B'$: performing a deadhead or trip directly before or after $b$ will result in a different block, possibly one which is not currently known. The same can be said when removing part of $b$: the resulting block will be different from $b$, and might currently still be unknown. Let $B_{alt,b}$ be the alternative blocks which can be created from $b$ by removing or adding a single trip or deadhead. Let us limit our possibilities by only allowing trips which can directly connect to $b$ with a short idle, and deadheads which can be connected without idle time.

Next, let $B_{alt} = \bigcup_{b \in B'} B_{alt,b}$. From $B_{alt}$, let us select some number of currently unknown blocks. We will now include these blocks in $B'$ before starting our column generation process; if an alternative block is useful for the crew solution, it will be used in the newly generated duties. If this occurs, we consider the block to be known, at which point we have dual costs which we can apply during a future vehicle task generation step. We have therefore now created a way for blocks which are good for crew solution quality to be created without relying on an unknown path in vehicle task generation. 
...
In order to prevent this from becoming an issue, an upper limit for the number of columns $N_{max,vt}$ and $N_{max,cd}$ were added for $VT$ and $CD$ respectively. Whenever $|VT| > N_{max,vt}$, sort all columns $vt \in VT$ on $cost_\lambda(vt)$ and discard the $|VT| - N_{max,vt}$ ones with the highest cost. When doing this, ensure that all trips can still be covered by at least one column, as otherwise no feasible solution can be found. The same can be done for removing the worst columns from $CD$, however here at least one column for each known block must be kept.

In addition to this, duplicate columns are manually filtered out. Primarily during the creation of secondary columns, columns which are identical to those already found earlier in the solving process are found; in order to guarantee a good convergence of the subgradient optimization and to keep the overall number of columns manageable, these columns were manually filtered out. For vehicle tasks, equality of columns was based on the covered trips and blocks. For crew duties, equality was based on the covered blocks and the duty type. If an equal column with lower costs was found, the previously found column was replaced with the new one.

\subparagraph{Crew labeling} Initial testing showed that when $|B'| \gtrapprox 200$, feasibility checks alone were no longer enough in order to fully propagate all required labels as the runtime of a single labeling round could exceed 10 minutes. In order to combat this, a two heuristics were added in order to reduce the overall number of propagations required.
